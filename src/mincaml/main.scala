...
object Main {
  ...
  def lexbuf(outchan:PrintWriter, inchan:Any):Unit = {
    ...
    val opt:KNormal.T = optimize(limit, alpha);println("optimize "+opt)
    val closure:Closure.Prog = Closure(opt); println("closure "+closure)
    val virtual:X86Asm.Prog = Virtual(closure); println("virtual "+virtual)
    val simm13:X86Asm.Prog = Simm13(virtual); println("simm13 "+simm13)
    val regialloc:X86Asm.Prog = RegAlloc(simm13); println("regalloc "+ regialloc)
